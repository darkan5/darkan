...
(function (factory) {
    ...
    var buildLayoutInfo = function ($editor) {
      ...
      if ($editor.hasClass('note-air-editor')) {
        ...
        makeFinder = function (sClassName) {
          //_log('looking for..', sClassName);
          return function () { 
            if (sClassName == '.note-toolbar') {
              return $('#menu-text').find('.note-toolbar'); 
            } else if (sClassName == '.note-dialog') {
              return $('.note-dialog');
            } else {
              //_log('znaleziono', $editor.find(sClassName));
        ...
        return {
          editor: function () { return window._textEditorOptions.componentEl.find('.note-editor'); },
          // dropzone: makeFinder('.note-dropzone'),
          toolbar: function () { return window._textEditorOptions.editorWrapper.find('.note-toolbar'); },
          editable: function () { return window._textEditorOptions.componentEl.find('.note-editable'); },
  ...
  var settings = {
    ...
    options: {
      ...
      // fontName
      fontNames: [
        'Lato', 'Alegreya', 'Arial', 'Average Sans', 'Advent Pro', 'Anaheim', 'Crete Round', 'Doppio One', 'Oxygen Mono', 'Quando', 'Righteous', 'Sanchez', 'Source Code Pro', 'Trykker', 'Futura'
      ],
  ...
  var Editor = function () {
    ...
    this.createLink = function ($editable, linkInfo, options) {
      ...
      $.each(anchors, function (idx, anchor) {
        $(anchor).attr({
          href: linkUrl,
          target: '_blank'//isNewWindow ? '_blank' : ''
        });
      });
    ...
    this.getLinkInfo = function ($editable) {
      ...
      return {
        range: rng,
        text: rng.toString(),
        isNewWindow: true,//$anchor.length ? $anchor.attr('target') === '_blank' : true,
  ...
  var Renderer = function () {
    ...
    var tplButtonInfo = {
      ...
      color: function (lang) {
        ...
        var colorButton = tplButton(colorButtonLabel, {
          className: 'note-color btn-group',
        ...
        var dropdown = '<ul class="dropdown-menu">' +
                         '<li>' +
                           ...
                           // '</div>' +
                           '<div class="btn-group">' +
                             '<div class="note-palette-title">' + _lang('FONT_COLOR') + '</div>' +
                             '<div class="note-color-reset" data-event="foreColor" data-value="inherit" title="' + _lang('FONT_COLOR_RESET') + '">' +
                               _lang('FONT_COLOR_RESET') +
                             '</div>' +
        ...
        var moreButton = tplButton('<i class="fa fa-font" style="color:#5082A5;"></i>', {
      ...
      paragraph: function (lang) {
        ...
        return '<div class="note-align btn-group">' +
                     leftButton + centerButton + rightButton + justifyButton +
                   '</div>';

        // return tplIconButton('fa fa-align-left hideme', {
      ...
      fontsize: function (lang) {
          ...
          var dropdown = '<ul class="dropdown-menu">' +
                   ...
                   '<li><a data-event="fontSize" data-value="16"><i class="fa fa-check icon-ok"></i> 16</a></li>' +
                   ...
                   '<li><a data-event="fontSize" data-value="28"><i class="fa fa-check icon-ok"></i> 28</a></li>' +
                   '<li><a data-event="fontSize" data-value="30"><i class="fa fa-check icon-ok"></i> 30</a></li>' +
                   '<li><a data-event="fontSize" data-value="32"><i class="fa fa-check icon-ok"></i> 32</a></li>' +
                   '<li><a data-event="fontSize" data-value="36"><i class="fa fa-check icon-ok"></i> 36</a></li>' +
                   '<li><a data-event="fontSize" data-value="42"><i class="fa fa-check icon-ok"></i> 42</a></li>' +
                   '<li><a data-event="fontSize" data-value="50"><i class="fa fa-check icon-ok"></i> 50</a></li>' +
                   '<li><a data-event="fontSize" data-value="66"><i class="fa fa-check icon-ok"></i> 66</a></li>' +
                 '</ul>'; 

          return tplIconButton('fa note-current-fontsize', {
            dropdown: dropdown
          });
    ...
    var tplDialogInfo = {
      ...
      link: function (lang, options) {
        var body = '<div class="form-group row-fluid">' +
                     '<label>' + _lang('INSERT_LINK_TXT_TO_DISPLAY') + '</label>' +
                     '<input class="note-link-text form-control span12" type="text" />' +
                   '</div>' +
                   '<div class="form-group row-fluid">' +
                     '<label>' + _lang('INSERT_LINK_URL') + '</label>' +
        ...
        var footer = '<button href="#" class="btn btn-primary note-link-btn disabled" disabled>' + _lang('INSERT_LINK') + '</button>';
        return tplDialog('note-link-dialog', _lang('INSERT_LINK'), body, footer);
      },
    ...
    this.createLayoutByFrame = function ($holder, options) {
      ...
      var menu_txt = $('#menu-text');

      menu_txt.find('.note-toolbar.btn-toolbar').remove();
      menu_txt.find('#text-editor-image').hide();



      var $toolbar = $(toolbarHTML);//.prependTo($editor);

      // var toolbar_clone = $toolbar.clone(true);
        $toolbar.appendTo(menu_txt);
      ...
      var body = document.body;
      //07. create Dialog
      var $dialog = $(tplDialogs(langInfo, options)).appendTo(body);
      $dialog.find('button.close, a.modal-close').click(function () {
        $(this).closest('.modal').modal('hide');
      });
    ...
    this.layoutInfoFromHolder = function ($holder) {
      ...
      //_log('layoutInfoFromHolder', layoutInfo);
      return layoutInfo;
    };
    ...
    this.removeLayout = function ($holder, layoutInfo, options) {
      ...
      // } else {
        ...
        layoutInfo.editor.off();

        layoutInfo.editor.remove();

//        layoutInfo.editor.css({
//          opacity: '0',
//          position: 'absolute',
//          top: '0px',
//          left: '0px',
...
//          height: '1px'
//        });
        //window._tempeditor = layoutInfo.editor;
        //layoutInfo.editor = window._tempeditor;
        $holder.show();
      // }
    };
  ...
  $.fn.extend({
    ...
    summernote: function (options) {
      ...
      window._textEditorOptions = options;
    ...
    destroySummernote: function () {
      ...
      //_log('destroy text', this, _log.error);

      this.each(function (idx, holder) {
        // _log('eachhhhhhhhhhhhhhhhhhhh');
        var $holder = $(holder);

          //_log('destroy $holder', $holder, _log.error);

        var info = renderer.layoutInfoFromHolder($holder);
        if (!info || !info.editable) { return; }

          //_log('destroy info', info, _log.error);
      ...
      window._textEditorOptions.editorWrapper.find('.note-toolbar').remove();
