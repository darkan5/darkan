var AsignUsersToMailView = PageView.extend({
    template: '#assign-user-to-mail-template',
    ...
    initialize: function(data) {
        ...
        this.model = new AssignUsersToMailsModel({groupId: this.groupId});
    ...
    selectUserToAssign: function(e) {
        ...
        var userMail = clickedRow.attr('umail');

        if (userMail) {
            clickedRow.toggleClass('success');

            if (this.selectedUsers.indexOf(userMail) === -1) {
                this.selectedUsers.push(userMail);
            } else {
                this.selectedUsers.splice(this.selectedUsers.indexOf(userMail), 1);
            }
        }
    ...
    assignSelectedUsers: function() {
        ...
        if (this.selectedUsers.length) {
            ...
            this.trigger('assign-selected-mails', this.selectedUsers);
    ...
    close: function(){
        $('#assign-users-window').modal('hide');
        this.remove();
    },
