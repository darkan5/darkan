...
Project.prototype.clearBeforeDisconnect = function(dir, list) {
    ...
    try{
        ...
        console.log('clearBeforeDisconnect error', ex.length);
...
Project.prototype.deleteHistory = function(){
    ...
    console.log('deleteHistory', this.sockets.sockets.length);

    try{
        var userId = '' + this.userId;
        var projectId = '' + this.projectId;
        if(this.sockets.sockets.length == 0){
...
Project.prototype.updateComponent = function(data, onResult, onFault, imageMagick ){
    ...
    try{
        ...
        projectModel.set('userId', this.socket.ownerId.toString());
        projectModel.set('projectId', this.socket.myRoom.toString());
...
Project.prototype.copyComponents = function( data, onResult, onFault ){
    ...
    try{
        ...
        var copyComponentVisitor = new CopyComponentVisitor(data, onResult, onFault);

        this.accept(copyComponentVisitor);

    } catch (ex){

        onFault({ error:'Copy components fault' });
...
Project.prototype.preparePreview = function( data, onResult, onFault ){
    var _that = this;
