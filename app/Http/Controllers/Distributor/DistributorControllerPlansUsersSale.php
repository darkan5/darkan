...
class DistributorControllerPlansUsersSale extends Controller
{
    ...
    public function getUsersList($start_date = false, $end_date = false, $info = '- aktualnie wybrana jest data od początku do końca tego miesiąca'){
    	...
    	$usersToDistributorsRabats = UsersToDistributorsRabats::where('user_id', '=', Auth::user()->id)
    									->where('start_date', '>=', $start_date)
    									->where('expiration_date', '<=', $end_date)
    									->where('currency_id', '=', $this->currency_id)
    									->get();

    	$maxRabat = UsersToDistributorsRabats::where('user_id', '=', Auth::user()->id)
    									->where('start_date', '>=', $start_date)
    									->where('expiration_date', '<=', $end_date)
    									->where('amount', '>=' , 0)
        ...
        return view('distributor.users.saled_plans_user_by_rabat_list')
        			->with('info', $info)
    ...
    private function getSailedPlansUserByPartnerUser($start_date, $end_date){
    	...
    	$reselersToDistributors = ReselersToDistributors::where('distributor_id', '=', Auth::user()->id)->get();

    	foreach ($reselersToDistributors as $reselerToDistributor) {
    		array_push($reselersIds, $reselerToDistributor->reseler_id);
    	}
    	...
    	$usersToDistributor = DB::table('users_to_reselers')
						...
						->whereIn('users_to_reselers.reseler_id', $reselersIds)
