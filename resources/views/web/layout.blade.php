...
<head>
	...
	<!-- <title><? //Lang::get('frontpage.PAGE_TITLE') ?></title> -->
...
<body>
	...
	smartsupp('email', "<?= Auth::check() ? Auth::user()->login : 'NotLoggedIn'?>");
	smartsupp('name', "<?= Auth::check() ? Auth::user()->login : 'NotLoggedIn'?>");
	smartsupp('language', "<?=$locale?>");
	smartsupp('variables', {
	    userName: { label: 'User name ', value: "<?= Auth::check() ? Auth::user()->login : 'NotLoggedIn'?>" },
		...
		<nav class="navbar topmenu navbar-default navbar-fixed-top scrolled">
			<div class="container-fluid">
				<div class="logo navbar-header">
					<a href="{{ config('app.protocol') }}{{ config('app.domain') }}{{ config('app.folder') }}">
						<img src="{{asset('/css/img/new_web/logo.png')}}">
					</a>
				</div>
				<div>
					<ul class="nav navbar-nav navbar-right">
						...
						@if (Auth::guest())
							<li><a href="{{ url('/login') }}"><i class="fa fa-key"></i> <?= Lang::get('frontpage.login') ?></a></li>
							<li class="hideonmobile"><a href="{{ url('/register') }}"><?= Lang::get('frontpage.register') ?></a></li>
						@endif
						...
						@if (!Auth::guest())
							...
							<li>
								...
								<ul class="dropdown-menu" role="menu">
									<li>
										<a class="subdomain-link" href="{{ config('app.protocol_not_secure') }}{{Auth::user()->subdomain}}.{{ config('app.domain') }}{{ config('app.folder') }}">
									...
									<li>
										<a href="{{ url('/darkanpanel') }}">
											<span><?=Lang::get('frontpage.PORTAL_MENU_LMS')?></span>
										</a>
									</li>
								</ul>
							</li>
							...
							<li>
								<a href="#" onClick="return false;" type="button" class="dropdown-toggle" data-toggle="dropdown">
									<span class="usermail" title="{{ Auth::user()->login }}">{{ Auth::user()->login }}</span> <span class="caret"></span>
								</a>
								<ul class="profile-menu dropdown-menu" role="menu">
									...
									<li>
										<a href="{{ url('/auth/logout') }}">
											<i class="fa fa-power-off"></i> <span><?=Lang::get('frontpage.logout')?></span>
										</a>
									</li>
								</ul>
							</li>
	...
	<!-- Scripts -->
    <!-- <script src="{{ asset('/editor/modules/utils/utils.js') }}"></script> -->
